...
from oldpy2store.base import Store, KvReader, AttrNames
from oldpy2store.util import lazyprop, num_of_args, attrs_of, wraps
from oldpy2store.utils.signatures import Sig, KO
...
def store_decorator(func):
    ...
    No, ``b`` is an instance of a ``oldpy2store.base.Store``, which is a class containing an
    instance of a store (here, ``a``).

    >>> type(b)
    <class 'oldpy2store.base.Store'>
    >>> b.store == a
    True
...
from oldpy2store.util import copy_attrs
from oldpy2store.errors import OverWritesNotAllowedError
...
class OverWritesNotAllowedMixin:
    ...
    Traceback (most recent call last):
      ...
    oldpy2store.errors.OverWritesNotAllowedError: key foo already exists and cannot be overwritten.
...
def kv_wrap_persister_cls(persister_cls, name=None):
    """Make a class that wraps a persister into a oldpy2store.base.Store,

    Args:
        persister_cls: The persister class to wrap

    Returns: A Store wrapping the persister (see oldpy2store.base)
...
def _wrap_outcoming(
    ...
    >>> from oldpy2store.trans import store_wrap
    >>> S = store_wrap(dict)
...
def add_path_get(store=None, *, name=None, path_type: type = tuple):
    ...
    See Also: `oldpy2store.key_mappers.paths.PathGetMixin`, `oldpy2store.key_mappers.paths.KeyPath`
    ...
    >>> # you need to involve more tools. Like oldpy2store.key_mappers.paths.KeyPath...
    >>> from oldpy2store.key_mappers.paths import KeyPath
    >>> from oldpy2store import kv_wrap
...
from oldpy2store.util import has_enabled_clear_method, inject_method, _delete_keys_one_by_one
