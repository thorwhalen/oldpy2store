...
def mk_cached_store(
    ...
    >>> from oldpy2store.caching import mk_cached_store
    >>> import time
    >>> class SlowDict(dict):
    ...     sleep_s = 0.2
...
def mk_sourced_store(
    ...
    >>> from oldpy2store.caching import mk_sourced_store
    >>>
    >>> class Local(dict):
    ...     def __getitem__(self, k):
...
from oldpy2store.util import has_enabled_clear_method
...
def mk_write_cached_store(
    ...
    >>> from oldpy2store.caching import mk_write_cached_store, ensure_clear_to_kv_store
    >>> from oldpy2store import Store
    >>>
    ...
    if not has_enabled_clear_method(w_cache):
        ...
        You can wrap w_cache in oldpy2store.trans.ensure_clear_method to inject a clear method, 
...
# from oldpy2store import StrTupleDict
#
