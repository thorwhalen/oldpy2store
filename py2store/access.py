...
respectively.
By default, they are:
    ~/.oldpy2store_configs.json and ~/.oldpy2store_defaults.json
respectively.
"""
import os
import importlib
...
from oldpy2store.util import str_to_var_str
from oldpy2store.sources import DictAttr
    ...
    if isinstance(f, str):
        if "." in f:
            *module_path, func_name = f.split(".")
            ...
            f = getattr(importlib.import_module("oldpy2store"), f)
...
try:
    ...
    user_configs_dirpath = os.path.expanduser(
        getenv("PY2STORE_CONFIGS_DIR", "~/.oldpy2store_configs")
    )
    ...
    if os.path.isdir(user_configs_dirpath):
        ...
        from oldpy2store.base import KvStore
        from oldpy2store.stores.local_store import (
            LocalJsonStore,
            LocalBinaryStore,
        )
        from oldpy2store.trans import wrap_kvs
        from oldpy2store.misc import MiscStoreMixin
        from functools import wraps

        if os.path.isdir(myconfigs_dirpath):
            from oldpy2store.mixins import OverWritesNotAllowedMixin
            ...
            warn(
                f"""The oldpy2store-myconfigs directory wasn't found: {myconfigs_dirpath}
        ...
        from oldpy2store.trans import add_ipython_key_completions
        ...
        user_configs_filepath = os.path.expanduser(
            getenv(
                "PY2STORE_CONFIGS_JSON_FILEPATH", "~/.oldpy2store_configs.json"
            )
        )
    ...
    user_defaults_filepath = os.path.expanduser(
        getenv("PY2STORE_DEFAULTS_JSON_FILEPATH", "~/.oldpy2store_defaults.json")
    )
