...
def add_attrs(remember_added_attrs=True, if_attr_exists="raise", **attrs):
    ...
    >>> from oldpy2store.util import add_attrs
...
def groupby(
    ...
    See Also: regroupby, itertools.groupby, and oldpy2store.source.SequenceKvReader
...
def regroupby(items, *key_funcs, **named_key_funcs):
    ...
    See Also: groupby, itertools.groupby, and oldpy2store.source.SequenceKvReader
...
def igroupby(
    ...
    """The generator version of oldpy2store groupby.
    Groups items according to group keys updated from those items through the given (item_to_)key function,
    ...
    >>> from oldpy2store import groupby
    >>>
