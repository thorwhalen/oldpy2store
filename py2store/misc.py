...
from oldpy2store.stores.local_store import LocalBinaryStore
from oldpy2store.slib.s_zipfile import FilesOfZip
from oldpy2store.slib.s_configparser import ConfigReader, ConfigStore
from oldpy2store.util import imdict
...
try:
    from oldpy2store.persisters.dropbox_w_urllib import bytes_from_dropbox
except Exception:
...
class MiscGetter:
    ...
    >>> from oldpy2store.misc import get_obj, misc_objs_get
    >>> import os
    >>> import json
    >>>
...
class MiscStoreMixin(MiscReaderMixin):
    ...
    See also: preset and postget args from wrap_kvs decorator from oldpy2store.trans.
...
class MiscGetterAndSetter(MiscGetter):
    ...
    >>> from oldpy2store.misc import set_obj, misc_objs  # the function and the object
    >>> import json
    >>> import os
    >>>
    >>> pjoin = lambda *p: os.path.join(os.path.expanduser('~'), *p)
