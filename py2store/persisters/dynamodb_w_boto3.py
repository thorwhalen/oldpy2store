...
class DynamoDbPersister(Persister):
    ...
    >>> from oldpy2store.persisters.dynamodb_w_boto3 import DynamoDbPersister
    >>> s = DynamoDbPersister()
    ...
    >>> len(s)
    0
    >>> s = DynamoDbPersister(table_name='oldpy2store2', key_fields=('name',))
    >>> for _key in s:
    ...   del s[_key]
    >>> len(s)
    0
