...
from oldpy2store.filesys import (
    FileCollection,
    validate_key_and_raise_key_error_on_exception,
)
from oldpy2store.util import ModuleNotFoundWarning
...
class AioFileBytesPersister(AioFileBytesReader, KvPersister):
    ...
    async def asetitem(self, k, v):
        ...
        >>> from oldpy2store.persisters.w_aiofile import AioFileBytesPersister
        >>> from oldpy2store.filesys import mk_tmp_oldpy2store_dir
        >>> import os
        >>>
        >>> rootdir = mk_tmp_oldpy2store_dir('test')
