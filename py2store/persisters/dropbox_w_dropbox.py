from oldpy2store.base import Persister
from oldpy2store.mixins import ReadOnlyMixin
...
class DropboxPersister(Persister):
    ...
    Finally, for the test below, you need to put this token in ~/.oldpy2store_configs.json' under key
    dropbox.__init__kwargs, and have a folder named /oldpy2store_data/test/ in your app space.

    >>> import json
    >>> import os
    >>>
    >>> configs = json.load(open(os.path.expanduser('~/.oldpy2store_configs.json')))
    >>> s = DropboxPersister('/oldpy2store_data/test/', **configs['dropbox']['__init__kwargs'])
    >>> if '/oldpy2store_data/test/_can_remove' in s:
    ...     del s['/oldpy2store_data/test/_can_remove']
    ...
    >>>
    >>> n = len(s)
    >>> if n == 1:
    ...     assert list(s) == ['/oldpy2store_data/test/_can_remove']
    ...
    >>> s['/oldpy2store_data/test/_can_remove'] = b'this is a test'
    >>> assert len(s) == n + 1
    >>> assert s['/oldpy2store_data/test/_can_remove'] == b'this is a test'
    >>> '/oldpy2store_data/test/_can_remove' in s
    True
    >>> del s['/oldpy2store_data/test/_can_remove']
    """
