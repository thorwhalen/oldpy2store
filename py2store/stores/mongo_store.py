...
from oldpy2store.persisters.mongo_w_pymongo import OldMongoPersister, MongoCollectionPersister
...
class MongoTupleKeyStore(MongoStore):
    ...
    >>> s = MongoTupleKeyStore(db_name='oldpy2store_tests', collection_name='tmp', key_fields=('_id', 'user'))
    >>> _ = s._mgc.remove({})
    >>> k = (1234, 'user')
    >>> v = {'name': 'bob', 'age': 42}
...
class MongoAnyKeyStore(MongoStore):
    ...
    >>> s = MongoAnyKeyStore(db_name='oldpy2store_tests', collection_name='tmp', )
    >>> for k in s: del s[k]
