...
from oldpy2store.errors import OverWritesNotAllowedError, DeletionsNotAllowed
from oldpy2store.test.util import get_s3_test_access_info_from_env_vars


# from collections.abc import MutableMapping
# from oldpy2store.base import AbstractObjStore
# import typing
#
...
#     from oldpy2store.stores.local_store import RelativePathFormatStore
#
...
def test_local_file_ops():
    ...
    from oldpy2store.stores.local_store import RelativePathFormatStore as cls

    _test_path_format_local_file_ops(cls)

    from oldpy2store.stores.local_store import RelativePathFormatStore2 as cls

    _test_path_format_local_file_ops(cls)


def test_dropbox():
    from oldpy2store.util import ModuleNotFoundIgnore

    with ModuleNotFoundIgnore():
        from oldpy2store.access import FAK
        from oldpy2store.stores.dropbox_store import DropboxTextStore
        import json
        import os

        try:
            filepath = os.path.expanduser(
                "~/.oldpy2store_configs/stores/json/dropbox.json"
            )
            configs = json.load(open(filepath))
            store = DropboxTextStore("/oldpy2store_data/test/", **configs[FAK]["k"])
            _multi_test(store)
    ...
    # from oldpy2store.kv import LocalFileStore as cls
    # _test_path_format_local_file_ops(cls)
...
#         # from oldpy2store.stores.s3_store import S3BucketStore, S3BucketStoreNoOverwrites
#         # from oldpy2store.stores.delegation_stores import S3Store
#         from oldpy2store.kv import S3Store
